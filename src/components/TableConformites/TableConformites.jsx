...
import { CONFORMITE_INPUT_TYPES, CONFORMITE_LABEL} from "../FormsModels/FormsModels"

class TableConformites extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  fetchData = () => {
    ...
    getData(`conformites?offset=${offset}&limit=${itemsPerPage}`)
      .then(response => {
  ...
  handlePagination = (pageNumber) => {
    ...
    handleSaveGet() {
        ...
        getData('conformites?'+this.get_query_parameters(formData.entries()))
            .then(response => {
                    let filtered_data = response.results.map(this.filter_headers);
  ...
  export default TableConformites;
...
import { INTERFACE_INPUT_TYPES, INTERFACE_LABEL} from "../FormsModels/FormsModels"

class TableInterfaces extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  fetchData = () => {
    ...
    getData(`interfaces?offset=${offset}&limit=${itemsPerPage}`)
      .then(response => {
  ...
  handlePagination = (pageNumber) => {
    ...
    parentStateHandler = (newState) => {
        ...
        getData('interfaces?nom_application=App_Test')
            .then(response => {
                    let filtered_data = response.results.map(this.filter_headers);
    ...
    handleSaveGet() {
        ...
        getData('interfaces?'+this.get_query_parameters(formData.entries()))
            .then(response => {
                    let filtered_data = response.results.map(this.filter_headers);
      ...
      return (
        <div className="table-acteurs-container">
          ...
          <div className="button-container">
            ...
            <FormPost onSave={this.handleSave} model={INTERFACE_INPUT_TYPES} label={INTERFACE_LABEL}/>
          </div>
  ...
  export default TableInterfaces;
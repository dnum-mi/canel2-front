...
import { MIGRATION_INPUT_TYPES, MIGRATION_LABEL } from "../FormsModels/FormsModels";

class TableMigrations extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  fetchData = () => {
    ...
    getData(`migrations?offset=${offset}&limit=${itemsPerPage}`)
      .then(response => {
  ...
  handlePagination = (pageNumber) => {
    ...
    parentStateHandler = (newState) => {
        ...
        getData('migrations?nom_application=App_Test')
            .then(response => {
                    let filtered_data = response.results.map(this.filter_headers);
    ...
    handleSaveGet() {
        ...
        getData('migrations?'+this.get_query_parameters(formData.entries()))
            .then(response => {
                    let filtered_data = response.results.map(this.filter_headers);
